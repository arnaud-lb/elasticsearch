...
/**
 * <p>A PrefixedShingleFilter constructs shingles (token n-grams) from a token stream.
 ...
 *
 * <p>This filter inserts the first token of the stream at the begining of every
 * shingle.</p>
 */
public final class PrefixedShingleFilter extends TokenFilter {
  ...
  /**
   * Constructs a PrefixedShingleFilter with the specified shingle size from the
   ...
   */
  public PrefixedShingleFilter(TokenStream input, int minShingleSize, int maxShingleSize) {
    super(input);
   ...
   * @param maxShingleSize maximum shingle size produced by the filter.
   */
  public PrefixedShingleFilter(TokenStream input, int maxShingleSize) {
    this(input, DEFAULT_MIN_SHINGLE_SIZE, maxShingleSize);
  }
  
  /**
   * Construct a PrefixedShingleFilter with default shingle size: 2.
   *
   * @param input input stream
   */
  public PrefixedShingleFilter(TokenStream input) {
    this(input, DEFAULT_MIN_SHINGLE_SIZE, DEFAULT_MAX_SHINGLE_SIZE);
  }

  /**
   * Construct a PrefixedShingleFilter with the specified token type for shingle tokens
   * and the default shingle size: 2
   *
   * @param input input stream
   * @param tokenType token type for shingle tokens
   */
  public PrefixedShingleFilter(TokenStream input, String tokenType) {
    this(input, DEFAULT_MIN_SHINGLE_SIZE, DEFAULT_MAX_SHINGLE_SIZE);
  ...
  public final boolean incrementToken() throws IOException {
    if (null == prefix) {
      InputWindowToken prefixWindow = getNextToken(null);
      if (null != prefixWindow) {
    ...
    if (inputWindow.size() >= gramSize.getValue()) {
      ...
      if ( ! isAllFiller && builtGramSize == gramSize.getValue()) {
        ...
        if (null == prefixLen) {
          prefixLen = offsetAtt.startOffset();
        }
        offsetAtt.setOffset(offsetAtt.startOffset() - prefixLen, nextToken.offsetAtt.endOffset() - prefixLen);
